...
namespace Core.Framework
{
    ...
    {
        [Inject]
        private readonly IPublisher<UpdateLoadingProgressSignal> _updateLoadingProgressPublisher;
        ...
        {
            _check = new CheckDownloadSize();
            _download = new DownloadDependency();
        ...
        public async UniTask<SceneInstance> LoadSceneAndActiveAsync(string sceneName, LoadSceneMode mode)
        {
            try
            {
                ...
                while (!handle.IsDone)
                {
                    ...
                    _updateLoadingProgressPublisher.Publish(signal);
